...
import com.garen.community.entity.Event;
import com.garen.community.event.EventProducer;
...
public class CommentController implements CommunityConstant {
    ...
    public String addComment(@PathVariable("discussPostId") int discussPostId, Comment comment) {
        ...
        if (comment.getEntityType() == ENTITY_TYPE_POST) {
            // 触发发帖事件
            event = new Event().setTopic(TOPIC_PUBLISH).setUserId(comment.getUserId()).setEntityType(ENTITY_TYPE_POST).setEntityId(discussPostId);
            eventProducer.fireEvent(event);
            // 计算帖子分数
